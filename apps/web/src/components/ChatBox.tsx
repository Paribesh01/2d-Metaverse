            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                {messages.length > 0 ? (
                    messages.map((message: any, index: number) => (
                        <div
                            key={index}
                            className={`flex flex-col ${message.email === userid ? 'items-end' : 'items-start'}`}
                        >
                            {/* Display email above the message */}
                            <div className="text-xs text-gray-500 mb-1">
                                {message.email}
                            </div>
                            <div
                                className={`max-w-xs px-4 py-2 rounded-lg ${message.email === userid
