...
interface Game {
    roomid: string;
    players: string[];
    message: { userId: string, postition: { x: number, y: number } }[];
    admin: string;
}
...
export class GameManager {
        ...
        ws.on('message', (message: string) => {
            ...
            switch (action) {
                case 'createRoom':
                    ...
                    break;
                case 'sendMessage':
                    this.sendMessage(userId, roomid, parsedMessage.position);
                    break;
                case 'joinRoom':
    ...
    sendMessage(userId: string, roomid: string, position: { x: number, y: number }) {
        const user = this.users.find(u => u.userId === userId);
        const game = this.games.find(g => g.roomid === roomid);

        if (game) {
            if (user && user.ws.readyState === WebSocket.OPEN) {
                game.message.push({ userId, postition: position });
                console.log("position", position);
                this.broadcastToRoom(roomid, { status: 'messageSent', userId, position });
